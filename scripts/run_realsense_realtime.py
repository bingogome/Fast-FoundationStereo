#!/usr/bin/env python3
"""Live RealSense D4xx IR stereo -> Fast-FoundationStereo disparity/depth/point cloud."""
...
def parse_args():
    ...
    parser.add_argument("--rectify", type=int, default=1, help="Rectify IR pair with camera calibration.")
    parser.add_argument("--equalize_hist", type=int, default=1, help="Apply histogram equalization on IR.")
    parser.add_argument("--scale", type=float, default=1.0, help="Inference image scale.")
    ...
    parser.add_argument("--pc_stride", type=int, default=2, help="Keep one point every N pixels.")
    ...
    parser.add_argument("--use_color", type=int, default=1, help="Colorize cloud with RGB stream.")
    ...
    parser.add_argument("--out_dir", type=str, default=f"{REPO_ROOT}/output/realsense")
    parser.add_argument("--log_every", type=int, default=30)
    parser.add_argument(
        "--frame_timeout_ms",
        type=int,
        default=5000,
        help="RealSense frame wait timeout in milliseconds.",
    )
    parser.add_argument(
        "--startup_retries",
        type=int,
        default=1,
        help="How many times to retry pipeline init before failing.",
    )
    parser.add_argument(
        "--restart_backoff_sec",
        type=float,
        default=0.8,
        help="Sleep before retrying/restarting pipeline.",
    )
...
def save_runtime_k(path, K, baseline):
    ...
    with open(path, "w", encoding="utf-8") as f:
        k_flat = " ".join(f"{x:.8f}" for x in K.reshape(-1))
        f.write(f"{k_flat}\n")
...
def create_realsense_config(rs, args):
    config = rs.config()
    if args.serial:
...
def start_pipeline_once(rs, args):
    with ExitStack() as cleanup:
        pipeline = rs.pipeline()
        profile = pipeline.start(create_realsense_config(rs, args))
        ...
        if not left_frame or not right_frame:
            raise RuntimeError("Failed to read RealSense IR streams.")
        ...
        if args.rectify:
            ...
            baseline = float(
                abs(left_profile.get_extrinsics_to(right_profile).translation[0])
            )
...
def start_pipeline_with_retries(rs, args, reason):
    last_exc = None
    for attempt in range(1, args.startup_retries + 1):
        try:
            pipeline, state = start_pipeline_once(rs, args)
            if attempt > 1:
                logging.info(
                    "RealSense pipeline recovered after %d attempts (%s).",
                    attempt,
                    reason,
                )
            return pipeline, state
    ...
    raise RuntimeError(
        f"Failed to start RealSense pipeline after {args.startup_retries} attempts ({reason})."
    ) from last_exc
...
def colorize_points_from_rgb(points_ir, color_bgr, K_color, R_ir_to_color, T_ir_to_color):
    ...
    uu[valid_z] = (
        K_color[0, 0] * pts_color[valid_z, 0] / z[valid_z] + K_color[0, 2]
    ).astype(np.int32)
    vv[valid_z] = (
        K_color[1, 1] * pts_color[valid_z, 1] / z[valid_z] + K_color[1, 2]
    ).astype(np.int32)

    h, w = color_bgr.shape[:2]
    in_bounds = valid_z & (uu >= 0) & (uu < w) & (vv >= 0) & (vv < h)
    if np.any(in_bounds):
        colors[in_bounds] = color_bgr[vv[in_bounds], uu[in_bounds], ::-1] / 255.0
    return colors


def maybe_save_snapshot(out_dir, left, right, disp_vis, depth):
    os.makedirs(out_dir, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    left_path = os.path.join(out_dir, f"left_{ts}.png")
    right_path = os.path.join(out_dir, f"right_{ts}.png")
    disp_path = os.path.join(out_dir, f"disp_{ts}.png")
    ...
    if not cv2.imwrite(disp_path, disp_vis[:, :, ::-1]):
        raise IOError(f"Failed to save image: {disp_path}")
    np.save(depth_path, depth)
    return ts, left_path, right_path, disp_path, depth_path


def maybe_save_point_cloud(out_dir, ts, points, colors):
    if len(points) == 0:
        return None

    if o3d is None:
        raise RuntimeError("open3d is required to save point cloud files.")

    cloud_path = os.path.join(out_dir, f"cloud_{ts}.ply")
    cloud = o3d.geometry.PointCloud()
    cloud.points = o3d.utility.Vector3dVector(points.astype(np.float64))
    cloud.colors = o3d.utility.Vector3dVector(colors.astype(np.float64))
    if not o3d.io.write_point_cloud(cloud_path, cloud):
        raise IOError(f"Failed to save point cloud: {cloud_path}")
    return cloud_path
...
def main():
    ...
    try:
        ...
        with ExitStack() as cleanup:
            ...
            pipeline, rs_state = start_pipeline_with_retries(rs, args, reason="initial startup")
            runtime_state["pipeline"] = pipeline
            rectify_maps = rs_state["rectify_maps"]
            K_ir = rs_state["K_ir"]
            baseline = rs_state["baseline"]
            K_color = rs_state["K_color"]
            R_ir_to_color = rs_state["R_ir_to_color"]
            T_ir_to_color = rs_state["T_ir_to_color"]
            ...
            if args.show_pc:
                if o3d is None:
                    raise RuntimeError("open3d not available but --show_pc=1 was requested.")
                else:
                    vis = o3d.visualization.Visualizer()
                    vis.create_window("FFS RealSense Point Cloud", width=1280, height=720)
            ...
            while True:
                ...
                if not left_frame or not right_frame:
                    raise RuntimeError("Missing IR frame during realtime capture.")
                ...
                if color_bgr is not None and K_color is not None:
                    ...
                    if len(colors) > 0:
                        ...
                        color_missing = np.isclose(colors.sum(axis=1), 0.0)
                        if np.any(color_missing):
                            colors[color_missing] = gray_rgb[color_missing]
                else:
                    gray = left_proc[valid_mask]
                ...
                if args.show_disp:
                    ...
                    cv2.imshow("FFS RealSense (left | right | disp)", panel[:, :, ::-1])
                    key = cv2.waitKey(1) & 0xFF
                    if key in (27, ord("q")):
                        break
                    if key == ord("s"):
                        ts, left_path, right_path, disp_path, depth_path = maybe_save_snapshot(
                            args.out_dir, left_proc, right_proc, disp_vis, depth
                        )
                        cloud_path = maybe_save_point_cloud(args.out_dir, ts, points_ir, colors)
                        if cloud_path is not None:
                            logging.info(
                                "Saved snapshot: left=%s right=%s disp=%s depth=%s cloud=%s",
                                left_path,
                                right_path,
                                disp_path,
                                depth_path,
                            ...
                            logging.info(
                                "Saved snapshot: left=%s right=%s disp=%s depth=%s (cloud skipped: no valid points)",
                                left_path,
                                right_path,
                                disp_path,
                                depth_path,
